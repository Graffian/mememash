...
export default function Home() {
  ...
  const googleSignIn = async () => {
    try {
      ...
      const userUpdate = await userInfo(
        response?.name,
        response?.email,
        response?.given_name,
        response?.picture,
    ...
    <>
      <div className={styles.landingPage}>
        <div className={styles.header}>
          <h1 className={styles.title}>MEMEMASH</h1>
          <p className={styles.subtitle}>
            Where memes battle for supremacy! 🔥
          </p>
          <p className={styles.description}>
            Upload your funniest memes and let the community decide which ones
            reign supreme through our epic voting system.
          </p>
        </div>

        <div className={styles.conceptDemo}>
          <h3 className={styles.howItWorks}>How it works:</h3>
          <div className={styles.memeVoting}>
            <div className={styles.memeCard}>
              <div className={styles.mockMeme}>🐱</div>
              <p>Meme A</p>
            </div>
          ...
          <p className={styles.voteExplanation}>
            Click on your favorite meme to boost its creator's ELO rating!
          </p>
        </div>

        <div className={styles.authSection}>
          <h3 className={styles.authTitle}>Join the Meme Wars!</h3>
          <div className={styles.signIn}>
            ...
            <button onClick={githubSignIn} className={styles.githubBtn}>
              <svg
        ...
        {name && (
          <div className={styles.welcomeSection}>
            <h3 className={styles.welcomeText}>Welcome back, {name}! 🎉</h3>
          </div>
        )}
      </div>
    </>
